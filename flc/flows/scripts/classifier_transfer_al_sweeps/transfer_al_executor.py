...
from flc.shared.transfer_active_learning.experiments import ClassifierExperimentConfig
from flc.shared.transfer_active_learning.experiments import ClassifierExperimentRunner
...
from .sweep_generator import ClassifierALCombination
from .config import DatasetTripletConfig


def execute_classifier_al(
    triplet_config: DatasetTripletConfig,
    combination: ClassifierALCombination,
    preprocessing_config: PreprocessingConfig,
    output_dir: str,
    max_flows: Optional[int] = None,
) -> Dict[str, Any]:
    """
    Execute classifier-based transfer active learning with specific output directory.
    ...
    (
        ...
        source_sample_ids,
        target_sample_ids,
    ) = load_and_prepare_datasets(triplet_config, preprocessing_config, max_flows, combination.random_state)
    ...
    # Create classifier AL configuration
    classifier_al_config = ClassifierActiveLearningConfig(
        samples_per_iteration=combination.samples_per_iteration,
        classifier_name=combination.classifier_name,
        classifier_config=combination.classifier_config or {},
        random_state=combination.random_state,
    )

    # Create experiment configuration
    experiment_config = ClassifierExperimentConfig(
        classifier_al_config=classifier_al_config,
...
def validate_al_inputs(
    ...
    if combination.samples_per_iteration <= 0:
...
def load_and_prepare_datasets(
    ...
    """
    Load datasets from triplet configuration and prepare for transfer AL.
    ...
    if preprocessing_config.enabled:
        source_dataset.set_preprocessor(config=preprocessing_config)
        target_dataset.set_preprocessor(config=preprocessing_config)
        test_dataset.set_preprocessor(config=preprocessing_config)

    # Convert to numpy arrays for transfer AL
    source_features, source_labels = source_dataset.to_sklearn_format(preprocessed=preprocessing_config.enabled)
    target_features, target_labels = target_dataset.to_sklearn_format(preprocessed=preprocessing_config.enabled)
    test_features, test_labels = test_dataset.to_sklearn_format(preprocessed=preprocessing_config.enabled)
