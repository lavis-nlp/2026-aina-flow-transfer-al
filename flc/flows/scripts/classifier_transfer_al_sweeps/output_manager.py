...
from flc.flows.scripts.classifier_transfer_al_sweeps.config import DatasetTripletConfig
...
def generate_directory_name(combination: ClassifierALCombination) -> str:
    """
...
def get_output_directory_structure(
    ...
    combinations: List[ClassifierALCombination],
) -> dict:
    """
...
def get_existing_results(
    ...
    for triplet_idx, _ in enumerate(dataset_triplets):
        ...
        for item in os.listdir(triplet_dir):
            ...
            learning_curve_file = os.path.join(combination_dir, "learning_curve.json")

            result_info = {
                "directory": combination_dir,
                "has_results": os.path.exists(results_file),
                "has_learning_curve": os.path.exists(learning_curve_file),
                "is_complete": os.path.exists(results_file) and os.path.exists(learning_curve_file),
            }
